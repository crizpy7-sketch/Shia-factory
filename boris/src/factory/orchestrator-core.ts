...
import { isVerifiedQualityGateReceiptResolution, validateCanonicalQualityGateReceipt,
  type QualityGateReceiptResolver } from '../quality/quality-gate.js';
...
export interface OrchestrationRequest {
  ...
  productionDeployment?: ProductionDeploymentRequest;
  migrationCapabilities?: MigrationCapabilityInput[];
  now: string;
}
...
export interface ProductionDeploymentRequest {
  ...
  qualityReceiptReferenceId?: string;
  approvalId?: string;
}

export interface TrustedProductionBaselineRecord {
  referenceId: string;
  repository: string;
  deployedSha: string;
...
export interface ProductionBaselineResolver {
  id: string;
  provenance: string;
  resolve(referenceId: string): TrustedProductionBaselineRecord | null;
}

export interface ProductionDeploymentDependencies {
  baselineResolver?: ProductionBaselineResolver;
  qualityReceiptResolver?: QualityGateReceiptResolver;
  governanceApprovalResolver?: GovernanceApprovalResolver;
}
...
export interface DeploymentPreconditionResult {
  ...
  state: 'satisfied' | 'missing' | 'unverified' | 'mismatch';
...
export interface ProductionDeploymentDecision {
  ...
  state: 'not-requested' | 'precondition-blocked' | 'eligible';
  candidateSha: string | null;
  preconditions: DeploymentPreconditionResult[];
  blockers: string[];
  productionMutationPerformed: false;
}

export interface MigrationCapabilityInput {
  capability: string;
  existingImplementation: 'working' | 'missing' | 'defective';
  evidence: string[];
  boundedPilotChangesCapability?: boolean;
  extractionCandidateAfterLifecycleProof?: boolean;
}

export interface MigrationCapabilityDecision {
  capability: string;
  factoryShelfDisposition: ShelfReuseDecision['disposition'];
  applicationAction: 'PRESERVE' | 'IMPLEMENT' | 'MODIFY';
  extractionAction: 'NONE' | 'CANDIDATE_AFTER_LIFECYCLE_PROOF';
  evidence: string[];
  reason: string;
}
...
export interface ActionDecision {
  ...
  allowed: boolean;
  executable: boolean;
  executionState: 'allowed' | 'gated' | 'precondition-blocked' | 'denied';
  blockers: string[];
}
...
export interface OrchestratorTaskContract {
  ...
  /** Present on contracts generated by the Phase 7-aware orchestrator; optional for stored v1 compatibility snapshots. */
  deployment?: ProductionDeploymentDecision;
  /** Present on contracts generated by the Phase 7-aware orchestrator; optional for stored v1 compatibility snapshots. */
  migration?: { analyzed: boolean; actions: MigrationCapabilityDecision[] };
...
export function productionBaselineDigest(record: Omit<TrustedProductionBaselineRecord, 'integrityDigest'>): string {
...
const CAPABILITY_ROLE: Array<[RegExp, string]> = [
  ...
  [/build|engineering|architecture|frontend|backend|database|postgres|supabase|ai|debug|devops|deploy|release/, 'boris'],
];
...
function toolSignals(profile: NormalizedAppProfile, request: OrchestrationRequest): string[] {
  ...
  const stackEvidence = Object.entries(profile.stack).flatMap(([key, value]) => [key, ...(Array.isArray(value) ? value : [value])])
    .join(' ').toLowerCase();
  const tools: string[] = [];
  ...
  if (/\bsupabase\b/.test(signal) || /\bsupabase\b/.test(stackEvidence)) tools.push('supabase');
...
function missingDeploymentPrecondition(id: DeploymentPreconditionId, reason: string): DeploymentPreconditionResult {
  return { id, state: 'missing', evidenceReference: null, reason };
}

function verifiedQualityReceipt(
  ...
  const resolution = resolver.resolve(referenceId);
  if (!resolution || !isVerifiedQualityGateReceiptResolution(resolution)
    || resolution.provenance.sourceId !== referenceId || !resolution.provenance.resolverId.trim()
    || !resolution.provenance.resolverProvenance.trim()) {
  ...
  if (receipt.receiptId !== referenceId || validateCanonicalQualityGateReceipt(receipt).length > 0) {
    return { id, state: 'unverified', evidenceReference: referenceId, reason: 'Trusted resolver returned a noncanonical or incomplete Quality Gate receipt.' };
  }
  if (receipt.schemaVersion !== '1.2.0' || receipt.evaluationScope !== 'pre-deployment-release-readiness'
    || receipt.receiptStatus !== 'current' || receipt.productionObservationRequirement !== 'required'
  ...
  return { id, state: 'satisfied', evidenceReference: referenceId, reason: 'Trusted Quality Gate pass is bound to the exact candidate.' };
}

export function evaluateProductionDeployment(
  profile: NormalizedAppProfile,
  request: OrchestrationRequest,
  dependencies: ProductionDeploymentDependencies = {},
): ProductionDeploymentDecision {
  if (!request.requestedActions.includes('deploy')) {
    return { requested: false, environment: null, existingApplication: false, state: 'not-requested', candidateSha: null,
      preconditions: [], blockers: [], productionMutationPerformed: false };
  }
  const declaration = request.productionDeployment;
  const deployment: ProductionDeploymentRequest = declaration ?? {
    environment: 'production', existingApplication: true, repository: profile.app.id, candidateSha: request.repository.commit,
  };
  const preconditions: DeploymentPreconditionResult[] = [];
  const baselineId = deployment.baselineReferenceId;
  const baselineResolver = dependencies.baselineResolver;
  const baseline = baselineId && baselineResolver ? baselineResolver.resolve(baselineId) : null;
  const baselineIntegrityValid = baseline !== null && baseline.referenceId === baselineId
    && /^[0-9a-f]{64}$/i.test(baseline.integrityDigest)
    && productionBaselineDigest((({ integrityDigest: _digest, ...rest }) => rest)(baseline)) === baseline.integrityDigest.toLowerCase()
    && baseline.repository === deployment.repository && !Number.isNaN(Date.parse(baseline.observedAt)) && baseline.collector.trim() !== '';

  const baselineResult = (
    id: DeploymentPreconditionId,
    predicate: (record: TrustedProductionBaselineRecord) => boolean,
    missingReason: string,
    mismatchReason: string,
  ): DeploymentPreconditionResult => {
    if (!baselineId) return missingDeploymentPrecondition(id, missingReason);
    if (!baselineResolver) return { id, state: 'unverified', evidenceReference: baselineId, reason: 'No trusted production-baseline resolver is configured.' };
    if (!baseline || !baselineIntegrityValid) return { id, state: 'unverified', evidenceReference: baselineId, reason: 'Production baseline provenance or integrity could not be verified.' };
    return predicate(baseline)
      ? { id, state: 'satisfied', evidenceReference: baselineId, reason: 'Trusted production baseline satisfies this precondition.' }
      : { id, state: 'mismatch', evidenceReference: baselineId, reason: mismatchReason };
  };
  preconditions.push(
    baselineResult('deployed-production-revision', (record) => /^[0-9a-f]{40,64}$/i.test(record.deployedSha),
      'Independently observed deployed production revision/SHA is missing.', 'Observed deployed production revision is invalid.'),
  ...
  if (!approvalId) preconditions.push(missingDeploymentPrecondition('cristian-deploy-approval', 'Cristian deployment approval is missing.'));
    ...
    const approval = admitGovernanceApprovalReference(approvalId, dependencies.governanceApprovalResolver);
    if (approval.state !== 'approved' || !isVerifiedGovernanceApproval(approval)) {
      preconditions.push({ id: 'cristian-deploy-approval', state: 'unverified', evidenceReference: approvalId,
        reason: approval.state === 'unverified' ? approval.reason : 'Governance approval could not be verified.' });
    } else if (approval.taskId !== request.taskId || approval.action !== 'deploy' || approval.candidateSha !== deployment.candidateSha) {
      preconditions.push({ id: 'cristian-deploy-approval', state: 'mismatch', evidenceReference: approvalId,
        reason: 'Cristian approval is not bound to this deploy action, task and exact candidate SHA.' });
    } else {
      preconditions.push({ id: 'cristian-deploy-approval', state: 'satisfied', evidenceReference: approvalId,
        reason: 'Trusted Factory governance approval is bound to this deploy action and exact candidate.' });
    }
  }

  const blockers = preconditions.filter((item) => item.state !== 'satisfied').map((item) => `${item.id}: ${item.reason}`);
  return { requested: true, environment: 'production', existingApplication: deployment.existingApplication,
    state: blockers.length === 0 ? 'eligible' : 'precondition-blocked', candidateSha: deployment.candidateSha,
    preconditions, blockers, productionMutationPerformed: false };
}

export function migrationCapabilityDecisions(
  inputs: MigrationCapabilityInput[] | undefined,
  shelfDisposition: ShelfReuseDecision['disposition'],
): MigrationCapabilityDecision[] {
  return (inputs ?? []).map((input) => {
    if (!input.capability.trim() || input.evidence.length === 0 || input.evidence.some((item) => !item.trim())) {
      throw new Error('migration capabilities require a name and evidence');
    }
    const applicationAction: MigrationCapabilityDecision['applicationAction'] = input.existingImplementation === 'missing'
      ? 'IMPLEMENT' : input.existingImplementation === 'working' && !input.boundedPilotChangesCapability ? 'PRESERVE' : 'MODIFY';
    return {
      capability: input.capability,
      factoryShelfDisposition: shelfDisposition,
      applicationAction,
      extractionAction: input.extractionCandidateAfterLifecycleProof ? 'CANDIDATE_AFTER_LIFECYCLE_PROOF' : 'NONE',
      evidence: [...input.evidence],
      reason: applicationAction === 'PRESERVE'
        ? `The application already has evidence of a working capability; Shelf ${shelfDisposition} does not authorize a rebuild.`
        : applicationAction === 'IMPLEMENT'
          ? `The application lacks this bounded capability; Shelf ${shelfDisposition} permits implementation with recorded no-match evidence.`
          : `The bounded pilot explicitly changes a defective or existing capability; preserve unaffected behavior while modifying it.`,
    };
  });
}
...
export function buildTaskContract(
  profile: NormalizedAppProfile,
  registries: OrchestratorRegistries,
  request: OrchestrationRequest,
  findings: ReuseFinding[],
  shelfDecision?: ShelfReuseDecision,
  productionDependencies: ProductionDeploymentDependencies = {},
  ...
  if (request.productionDeployment) {
    if (request.productionDeployment.candidateSha !== request.repository.commit) {
      throw new Error('productionDeployment.candidateSha must match repository.commit');
    }
    if (!/^[0-9a-f]{40,64}$/i.test(request.productionDeployment.candidateSha)) {
      throw new Error('productionDeployment.candidateSha must be an exact Git SHA');
    }
    if (!request.productionDeployment.repository.trim()) throw new Error('productionDeployment.repository is required');
  }
  ...
  const deployment = evaluateProductionDeployment(profile, request, productionDependencies);
  const allowedActions: ActionDecision[] = request.requestedActions.map((action) => {
    ...
    const deployBlocked = action === 'deploy' && deployment.state === 'precondition-blocked';
    const gated = authority.startsWith('gated:');
    const satisfiedProductionGate = action === 'deploy' && deployment.state === 'eligible';
    return { action, role, authority, allowed, executable: allowed && !deployBlocked && (!gated || satisfiedProductionGate),
      executionState: !allowed ? 'denied' : deployBlocked ? 'precondition-blocked' : gated ? 'gated' : 'allowed',
      blockers: deployBlocked ? [...deployment.blockers] : [] };
  });
  ...
  certificationReleaseBlockers.push(...deployment.blockers.map((blocker) => `production deploy precondition: ${blocker}`));
  ...
  const migrationActions = migrationCapabilityDecisions(request.migrationCapabilities, shelfDecision?.disposition ?? 'CREATE');
  ...
  return {
    ...
    allowedActions, approvalGates: [...approvalGates].sort(), deployment,
    migration: { analyzed: migrationActions.length > 0, actions: migrationActions },
...
export function createDecisionReceipt(profile: NormalizedAppProfile, contract: OrchestratorTaskContract, request: OrchestrationRequest): DecisionReceipt {
  const decisions = [
    ...
    ...(contract.deployment?.requested ? [{ stage: 'production-deployment', decision: contract.deployment.state,
      reason: contract.deployment.blockers.join('; ') || 'All trusted production deployment preconditions are satisfied.',
      evidence: contract.deployment.preconditions.flatMap((item) => item.evidenceReference ? [item.evidenceReference] : []) }] : []),
    ...(contract.migration?.analyzed ? [{ stage: 'application-migration',
      decision: contract.migration.actions.map((item) => `${item.capability}:${item.applicationAction}`).join(','),
      reason: 'Application migration actions are separate from Factory Shelf dispositions.',
      evidence: contract.migration.actions.flatMap((item) => item.evidence) }] : []),
...
export async function orchestrate(
  ...
  productionDeploymentDependencies: ProductionDeploymentDependencies = {},
  ...
  const contract = buildTaskContract(profile, registries, request, findings, shelfDecision, productionDeploymentDependencies);
